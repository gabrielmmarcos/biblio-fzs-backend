...
from biblio_fzs_backend.services.curso_service import create_cursos_service, matricular_aluno_funcionario_service, matricular_aluno_com_aluno_autenticado_service, get_curso_by_id_service, delete_curso_service
...
async def get_curso_by_id(
    ...
    return await get_curso_by_id_service(session, id_curso)

@router.delete('/delete_curso/{id_curso}/', response_model=Message)
async def delete_curso(
    id_curso: int,
    session: AsyncSession = Depends(get_session),
):
    return await delete_curso_service(session, id_curso)